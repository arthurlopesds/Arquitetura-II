...
with open("fib.txt", "r") as arquivo: #leitura do arquivo que contem o codigo assembly
    codigo = arquivo.readlines() #readlines coloca cada linha do codigo em um indice do array codigo

with open("mem.txt", "r") as arquivo_memoria: #leitura do arquivo de memoria que contem os valores que serão usados
...
def Busca_Memoria(indice):
    valor = Busca_Cache(0,indice-1)

    if valor == -1:
        Escreve_Cache(0,indice-1,int(mem[indice-1]))
        return mem[indice-1]
...
def Busca_Instrucao(reginstrucao):
    ...
    inst = Busca_Cache(1,reginstrucao)

    if inst == -1:
        Escreve_Cache(1,reginstrucao,codigo[reginstrucao])
...
def Busca_Cache(tag,End):

    global hit,miss

    if tag == 0:
        for i in range(len(cache)):
            if cache[i].ObterTag() == 0 and cache[i].ObterEnd() == End:
                hit+=1
                return cache[i].ObterValMem()

    elif tag == 1:
        for i in range(len(cache)):
            if cache[i].ObterTag() == 1 and cache[i].ObterEnd() == End:
                hit+=1
                return cache[i].ObterValInst()
    miss+=1
    return -1

def Escreve_Cache(tag,End,val):

    vazio = False
    global pos
    for i in range(len(cache)):
        if cache[i].ObterTag() == -1:
            cache[i].tag = tag
            cache[i].end = End
            if tag == 0:
                cache[i].valMem = val
    ...
    if vazio == False:
        ...
        cache[pos].end = End
        if tag == 0:
            cache[pos].valMem = val
        else:
            cache[pos].valInst = val
        pos = (pos+1) % len(cache)
...
def Executa_Instrucao():
    ...
    if opcao == 1:
        ...
        if type(parametros[0]) != int: #se for diferente do tipo Inteiro
            ...
            Escreve_Memoria(int(parametros[0]), str(parametros[1])+'\n') #se nao for, escreve o valor no determinado endereço
    ...
    #executa o JE
    	...
    	#Verifica se o resultado do CMP foi igual a zero
        if resultado == 0:
    ...
    #executa o JL
    	...
    	#Verifica se o resultado do CMP foi menor que zero
        if resultado < 0:
            for i in range(len(pjumpInst)): #do tamanho do array de labels
                if pjumpInst[i] == (parametros[0]+":"):  #comparando de acordo com os labels que tão no array
                    ir = pjumpIR[i] #fazendo o jmp
    ...
    #executa o JG
    	...
    	#Verifica se o resultado do CMP foi maior que zero
        if resultado > 0:
            for i in range(len(pjumpInst)): #do tamanho do array de labels
                if pjumpInst[i] == (parametros[0]+":"):  #comparando de acordo com os labels que tão no array
                    ir = pjumpIR[i] #fazendo o jmp
    ...
    # executa o MUL
        ...
        if type(parametros[0]) != int:
        	#Se o primeiro parâmetro é um registrador, o resultado da multiplicação fica no registrador indicado
        	...
        	#Se o primeiro parâmetro é um endereço de memória, o resultado da multiplicação fica no endereço de memória indicado
    ...
    # executa o DIV
        ...
        # Verifica se o primeiro parâmetro da instrução é um registrador ou memória
        if type(parametros[0]) != int:
        	#Se o primeiro parâmetro é um registrador, o resultado da divisão fica no registrador indicado
        	#O resto da divisão sempre vai ficar no registrador 5
        	...
        	#Se o primeiro parâmetro é um endereço de memória, o resultado da divisão fica no endereço de memória indicado
        	#O resto da divisão sempre vai ficar no registrador 5
        	reg[4] = Busca_Memoria(int(parametros[0])) % int(parametros[1])
...
Zera_Registrador()#Coloca todos os registradores para zero
...
print('\nA instrução levou {} Ciclos de Clock '.format(ciclos)) #Print para a quantidade de ciclos que foi necessário para
                                                                #executar o codigo
print('MISS = {}  |  HIT  = {}'.format(miss,hit))